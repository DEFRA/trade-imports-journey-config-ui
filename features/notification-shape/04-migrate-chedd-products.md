...
- **CED-specific validation.** `Commodities.commodityIntendedFor` is a required (`@NotNull`) field under the CED validation groups. `Purpose.internalMarketPurpose` validates under an unrelated internal-market purpose group — it is the animals `subPurpose` field.
