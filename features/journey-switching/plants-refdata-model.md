...
The file is produced by an upstream field-config analysis pipeline that
distils source data into JSON. The domain reference for the data is
`src/server/journeys/chedpp-plants/research.md`.

The data has **two natural grains**, and that distinction is the single
most important fact about it:

| Grain                                                                | Facts                                                                                             |
| -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| **commodity code** (8-digit, e.g. `0808108090`)                      | group; `requires_test_and_trial`; `requires_finished_or_propagated`; propagation; quality classes |
| **species** = `commodity_code\|eppo_code` (e.g. `0808108090\|MABSD`) | regulatory authority (PHSI/HMI/JOINT); marketing standard (GMS/SMS); validity period; varieties   |
...
Distribution: the overwhelming majority of species are PHSI (no marketing
implications); only a small minority carry HMI/JOINT marketing data; and
varieties are very sparse. See `research.md` for the full picture of the
skew and its design consequences.

## The current flat shape, and why it's incoherent

The upstream build emits three top-level keys at the two grains, but
**denormalises** them:
...
```jsonc
{
  ...
  "definitions": {
     "varieties": { "<code>|<eppo>": ["Cavendish", ...] },  // distinct lists
     "groups":    { "<code>": "Fruit and nuts" },           // code -> name
...
1. **Commodity-grain flags are copied onto every species row.**
   ...
   routing entry. A commodity with 75 species stores them 75 times.
2. **`has_gms` is a derived boolean, and a misnomer.**
   `has_gms = marketing_standard != null` — it's `true` for _SMS_ too, so
   it means "has a marketing standard," not "is GMS." The resolver treats
   it as the GMS-declaration trigger. The correct rule (HMI + GMS) is
   documented in `research.md`; normalisation preserves today's behaviour
   exactly, and Phase B applies the correction.
3. **`has_varieties` is derived** — exactly `varieties[key]` presence.
   Storing it duplicates a fact.
4. **`requires_billing` is a rule, not data** — constant `true` for
   species, `false` for the fallback row.
5. **`classes` is disconnected** — the per-commodity class mapping was
   dropped; `classes` is a static 3-value literal with no linkage to
   commodities, so the variety/class page data can't be reconstructed.
...
- The GMS-declaration trigger is **derived at read time** from
  `regulatory_authority` and `marketing_standard` (the HMI + GMS rule in
  `research.md`). Normalisation keeps the current derivation
  (`marketing_standard != null`) so behaviour is unchanged until Phase B
...
const lookupRouting = (refdata, commodity) => {
  ...
  return {
    has_gms: sp?.marketing_standard != null, // preserved as-is; correct rule applied in Phase B
...
- The PHSI-only species rows: they add no variance (just "this species
  exists and is PHSI"), so they are not enumerated — a commodity is
  PHSI-only when it has no `species` entries.
