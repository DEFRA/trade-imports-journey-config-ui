...
> **Prerequisite:** Read `plants-refdata-model.md` first — it is the
> contract this story implements. The verified GMS rule, the gap it
> closes, and the per-scenario re-pin impact are summarised below and in
> `src/server/journeys/chedpp-plants/research.md`.
...
1. **Preflight — normalise the data.** Replace the flat denormalised
   ...
   per-commodity class linkage that the original build dropped, and remove
   the stored derived flags (`has_gms`/`has_varieties`/`requires_billing`).
   Behaviour-preserving: all seven committed scenarios keep their pinned
   counts after Phase A.
...
(true for SMS); and per-commodity class data was dropped at build time.
Surfacing this data in story 02 without first making it coherent is what
the user explicitly ruled out.

Independently, the verified IPAFFS rule is _any species with `HMI` +
`GMS`_. The current resolver fires for every marketing-bearing species
(including all JOINT and all SMS) — a large over-trigger, almost all
JOINT — which Phase B corrects.
...
- **Producer of today's refdata:** an upstream field-config analysis
  pipeline (a separate repo). We do **not** edit it; reconstruction
  happens **here**.
...
- **Sources to vendor** (not depend on live): a `commodity_class`
  snapshot, vendored into the one-shot migration tooling. The species
  sources are not needed — Phase A reconstructs from the committed flat
  file.
...
`commodity_class` snapshot revision), and counts.

#### A4. Restore the `classes` linkage

`commodity_class` is `(commodity_code, class)`. **Pin a specific
snapshot revision deliberately** (don't just take "highest") and record
it in `_meta`; confirm its commodity codes resolve against `commodities`
and its row count matches expectations. Group by commodity code into a
**deterministically ordered** class list (use the source `display_order`
...
```bash
# HMI+GMS candidates: the canonical positive case
...
# JOINT+GMS candidates (pick one for representativeness)
...
same way `importPeppers` already overrides authority+standard in
`scenarios.js`), so the fixture is self-describing.
...
- [ ] `chedpp-plants/README.md` updated: the "Implementation history"
      section reads in the past tense.
- [ ] Full `npm test` green.
...
| R3  | The vendored `commodity_class` revision is inconsistent with the group/config era used by the original build.                       | Pin the revision deliberately; record in `_meta`; assert its codes resolve against `commodities` and its expected row count (A4).                                                                                |
...
- The upstream refdata build script — left as provenance.
...
- **Closes the GMS-rule correction** (the verified HMI+GMS predicate),
  plus the data-coherence concerns raised on the original
