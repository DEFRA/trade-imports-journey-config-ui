...
- **dimension** — a _variance-annotated_ value list (common/specific
  ...
  Commodity group.
- **detail** — a _labelled group of rows shown as-is_, no variance.
...
has to _gate commodity-config off_ for non-animals journeys — an interim
...
**Parity rule (settled with the user):** the page _may_ change shape;
...
- Refdata shapes:
  - **animals** (unchanged): `content[k]`: `purpose`, `identifiers`,
    `quantity` (each a _name_ dereferenced via `definitions`);
  ...
  - **Validated dimension signal:** `regulatory_authority` → JOINT
    dominant, HMI specific; `marketing_standard` → GMS dominant, SMS
    specific; `validity_period` → a handful of distinct values that vary
    across commodities; `group` (commodity-grain) → 11 distinct values
    (Vegetables dominant). All four carry real common/specific signal —
    all four are dimensions.
- **Dropdown source.** `config-utils#extractCommodityOptions` currently
  reads `refdata.routing` keys (the full commodity/species key set for
  plants in the old shape). The
...
- **Every dimension yields a list of values per commodity.** Animals'
  ...
  difference — and is why the descriptor must be a _function_, not config.
- **`sourceFor` is optional** and exists only so animals can keep showing
  the _set name_ it currently displays (e.g. "standard_purposes"). Plants
...
export const refdataView = (refdata) => {
  ...
  const com = (field) => (k) =>
    [commodities[codeOf(k)]?.[field]].filter(Boolean)
  return {
    dimensions: [
      // species-grain
      {
      ...
      { id: 'group', name: 'Commodity group', valuesFor: com('group') }
    ],
    details: [
      {
        id: 'commodity_flags',
...
- **Dropdown source becomes journey-aware.** `extractCommodityOptions`
  ...
  - Plants: union of `Object.keys(refdata.species)` (the species
    keys) and the PHSI-only commodity codes from `Object.keys(refdata.commodities)`
    that have no species rows (the commodity-only entries) — together
    these match the key set the old `routing` had, so the dropdown
...
- **Dimensions** — the `excluded` list (superset − present) is always
  rendered when non-empty, so a value the journey _can_ have but this
  commodity _lacks_ is visible. (Already the behaviour; preserved.)
