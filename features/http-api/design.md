...
| I3  | Cross-page notification state is preserved end-to-end ("load scenario → edit on debug → tasklist reflects the edit"). The _mechanism_ moves from a side-effecting proxy to an explicit `PUT /ui/session/notification`, but the user-visible behaviour is identical. |
