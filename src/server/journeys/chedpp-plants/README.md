...
Each row of the table below is a distinct `(commodity_code, eppo_code)`
combination. The authority × marketing-standard cell decides whether the
GMS declaration page fires:

| Authority | Marketing standard | GMS page fires? | Notes                                                                                                                          |
| --------- | ------------------ | --------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| PHSI      | — (absent)         | no              | Phyto-only baseline. No marketing process. The dominant case.                                                                  |
| HMI       | GMS                | **yes**         | **The only combination that fires the GMS page.**                                                                              |
| HMI       | SMS                | no              | HMI inspects, but the _Specific_ rules apply — handled outside the GMS declaration.                                            |
| JOINT     | GMS                | no              | Both authorities inspect, but the GMS page is gated strictly on `HMI`, not `JOINT`. JOINT cases route through a different flow. |
| JOINT     | SMS                | no              | Same JOINT routing; Specific standard not via GMS declaration.                                                                 |

Only **HMI + GMS** species-pairs trigger the GMS declaration page —
everything else (the PHSI baseline, HMI+SMS, and all JOINT cases) does not.
...
JOINT+GMS — route through a different document-code path, not the GMS
declaration page.

## Implementation history

Pre-Story-03 the adapter's GMS-declaration predicate was the
over-permissive `marketing_standard != null` derivation, which fired for
every marketing-bearing species — HMI, JOINT, GMS and SMS alike. Story 03
Phase B replaced it with the verified predicate
`regulatory_authority === 'HMI' && marketing_standard === 'GMS'`, which
fires only for HMI+GMS species — a large correction that removed the JOINT
majority. The flag is no longer stored — `requiresGmsDeclaration` reads the
species record directly.
...
- `research.md` (this directory) — the domain reference and the
  GMS-declaration predicate.
